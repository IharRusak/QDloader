...
int MergeNV(uint8_t * lpCodeSrc, uint32_t dwCodeSizeSrc, uint8_t * lpCodeDest, uint32_t dwCodeSizeDest)
{
    ...
    if (bMultiCoreSrc)
    {
        ...
        for (i=0; i<le16toh(pCoreHeaderSrc->wCountCore); i++)
        {
            ...
            {
                if (!MergeNormalNV(lpCodeSrc+le32toh(pItemSrc->dwDataOffset), le32toh(pItemSrc->dwDataSize),
                    lpCodeDest+le32toh(pItemDest->dwDataOffset), le32toh(pItemDest->dwDataSize), 1))
                {
