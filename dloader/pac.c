...
int pac_parser( pac_ctx_t *ctx, FILE *fw_fp)
{
    ...
    if (le32toh(pachdr->nFileCount) > 64)
        return -1;

    n = fread(ctx->bin_hdr, 1, sizeof(struct bin_header_t) * le32toh(pachdr->nFileCount), fp);
    if (n != (sizeof(struct bin_header_t) * le32toh(pachdr->nFileCount)))
         return -1;
    ...
    for (n = 0; n < le32toh(pachdr->nFileCount); n++)
    {
        struct bin_header_t *bin_hdr = &ctx->bin_hdr[n];
        ...
        if (udx710 == 1 || uic8310 == 1 || uic8850 == 1)
        {
            bin_hdr->_nFileFlag = le32toh(bin_hdr->_nFileFlag);
            bin_hdr->_nCheckFlag = le32toh(bin_hdr->_nCheckFlag);

            if (verbose > 0) dprintf("idx: %zd, FileID: %s, FileName: %s, szFileVersion: %s, Size: %u, _nFileFlag:%u,_nCheckFlag:%u\n",
                n, (char *)(bin_hdr->szFileID), (char *)(bin_hdr->szFileName),
