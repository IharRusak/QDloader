...
static int  test_bsl(pac_ctx_t *ctx ) {
    ...
    for (i = 0; i < ctx->SchemeNum; i++)
    {
        ...
        if (!strcasecmp(x->FileID, "HOST_FDL")) {
            ...
            if (uic8850)
            {
                ...
                else
                {
                    ...
                    frame_sz = FRAMESZ_PDL;    //8850  FDL 0x210   other 0x800
                }
            }
        }
            ...
            if (uix8910)                    //8850 no FDL2, 8310 have FDL2?
                frame_sz = FRAMESZ_PDL;
            else
            {
                int tmp = ql_get_kernel_version();
        ...
        else if (udx710 && (strcmp(x->FileID,"ProdNV") == 0 || strcmp(x->FileID,"PhaseCheck") == 0))
        {
        ...
        else if (uic8310 && x->CheckFlag[0] == '0') {
            dprintf("skip FileID: %s\n", x->FileID);
        }
        else if (uic8850 && x->CheckFlag[0] == '0') {
            dprintf("skip FileID: %s\n", x->FileID);
        }
        ...
        else if (!strcasecmp(x->Type, "EraseFlash2"))
        {
			...
			<File>
                ...
                else if (!strcasecmp(x->Type, "EraseFlash")) {
                    bsl_exec(bsl_erase_id(ctx, x));
                    continue;
                }
            }
...
int dloader_main(int usbfd, FILE *fw_fp, const char *modem_name)
{
    ...
    else if (!strcmp(modem_name, "EC800G")) {
        uic8850 = 1;
    }
